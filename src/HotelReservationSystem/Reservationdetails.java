...
public class Reservationdetails extends javax.swing.JFrame {
    ...
    //Event Handler for Mouse Click event on the "btn_update"
    private void btn_updateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_updateActionPerformed
        ...
        try 
        {
            ...
            no_adult = Integer.parseInt(txt_no_of_adults.getText());  //INPUT VALIDATIONS
            no_child = Integer.parseInt(txt_no_of_child.getText());

            //VALIDATIONS FOR ROOM ID
            if (roomid == null) {
                ...
                try {
                    ...
                    tbl_reservation.clearSelection();

                } catch (NullPointerException e) {
                    JOptionPane.showMessageDialog(null, "Something went wrong ", "Error", JOptionPane.ERROR_MESSAGE);
                } catch (Exception e) {  //Generic Exception for any kind of exception
                    JOptionPane.showMessageDialog(null, "Something went wrong ", "Error", JOptionPane.ERROR_MESSAGE);
                }
    ...
    //Event Handler for Button Click Event on btn_back
    private void btn_backActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_backActionPerformed
        this.dispose(); //Disposes the current page
    }//GEN-LAST:event_btn_backActionPerformed

    //Event Handler for Mouse Click Event to select the row that needs to be updated/deleted
    private void tbl_reservationMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbl_reservationMouseClicked
        int selectedRow = tbl_reservation.getSelectedRow(); //Get the index of the selected row
        TableModel model = tbl_reservation.getModel();  //Establish the table model that represents the structure & data of the table

        //VALIDATIONS TO ENSURE THAT USER INTERCTS WITH THE SELECTED DATA
        txt_no_of_adults.setText(model.getValueAt(selectedRow, 3).toString()); 
        //Set the No of Adults text field to the value from the selected row
        ...
        //Retrieve the data from the First Column(index 0)of the selected row & store it in roomid
        ...
        try {
            Date checkin = sfd.parse(checkindate); //Handle the parsing of the Checking & Checkout dates
    ...
    //Event Handler for Button Click Event to Delete a Data Tuple
    private void btn_deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_deleteActionPerformed
        try {
            Statement st = HotelReservationSystem.DBConnection.creatConnection().createStatement();
            String query = "DELETE FROM `room_reservation` WHERE `room_reservation`.`reservation_id` = '" + reservID + "';";
            int count = st.executeUpdate(query);  //executeUpdate Method call to execute the DELETE query & returns the No of rows deleted
            //System.out.println(query);
            //DATA VALIDATIONS 
            if (count > 0) {
                JOptionPane.showMessageDialog(null, "Reservation delete successfull.", "Success", JOptionPane.INFORMATION_MESSAGE);
                setData();  //Refresh the table after deleting the selected tuple
                clear();  //Clear the input fields

            }

            tbl_reservation.clearSelection();  //Deselect previous selected rows
    ...
    //VALIDATIONS FOR CLEAR FORM
    public void clear() {
        txt_no_of_adults.setText(null);  //Sets the textfields to null, clearing the contents inside them
    ...
    public void setData() {
        ...
        tbl_reservation.removeAll();
        tbl_reservation.repaint();

        DefaultTableModel s = (DefaultTableModel) tbl_reservation.getModel();
        s.setNumRows(0);
